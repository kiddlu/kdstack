...
//i686-w64-mingw32-gcc -o erasedrv erasedrv.c -lsetupapi -static
...
static inline void infoLog(TCHAR * fstring, ...) {
	va_list args;
...
static inline void errorLog(TCHAR * fstring, ...) {
	va_list args;
	va_start(args, fstring);
	if (_vstprintf(logBuffer, fstring, args) < 0) {
		_stprintf(logBuffer, "");
...
static inline void warnLog(TCHAR * fstring, ...) {
	va_list args;
	va_start(args, fstring);
	if (_vstprintf(logBuffer, fstring, args) < 0) {
		_stprintf(logBuffer, "");
...
int removeOEMFiles(TCHAR * oemInfFile) {
	...
	TCHAR * oemPnfFile = (TCHAR *)malloc(sizeof(TCHAR) * (oemFilePathLength + 1)); //including null terminating
