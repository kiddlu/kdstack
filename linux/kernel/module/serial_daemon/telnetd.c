...
#ifdef SERIAL_DAEMON
int serial_daemon_sendmsg(int nl_fd, struct sockaddr_nl *dst_addr, unsigned int cmd)
{
	...
	nlh_send=(struct nlmsghdr *)malloc(NLMSG_SPACE(MAX_MSGSIZE));
	nlh_send->nlmsg_len = NLMSG_SPACE(MAX_MSGSIZE);
	nlh_send->nlmsg_pid = getpid();
	nlh_send->nlmsg_flags = cmd;
...
void serial_daemon_recvmsg(int nl_fd)
{
	if(!G.sessions) 
	{
		return;
	...
	nlh_recv=(struct nlmsghdr *)malloc(NLMSG_SPACE(MAX_MSGSIZE));
	memset(nlh_recv, 0, NLMSG_SPACE(MAX_MSGSIZE));
	...
	{
		printf("connnect close by server!\n");
	}
	else
	{
		msg_len = nlh_recv->nlmsg_flags;
		iac_safe_write(G.sessions->sockfd_write, NLMSG_DATA(nlh_recv), msg_len);  
	} 
	free(nlh_recv);
}
#endif /* SERIAL_DAEMON */
...
#ifdef SERIAL_DAEMON
	int netlink_fd;
	struct sockaddr_nl src_nl_addr, dst_nl_addr;
#endif
...
#ifdef SERIAL_DAEMON
	memset(&src_nl_addr, 0, sizeof(struct sockaddr_nl));
	src_nl_addr.nl_family = AF_NETLINK;
	src_nl_addr.nl_pid = getpid(); 
	src_nl_addr.nl_groups = 0;

	memset(&dst_nl_addr, 0, sizeof(struct sockaddr_nl));
	dst_nl_addr.nl_family = AF_NETLINK;
	dst_nl_addr.nl_pid = 0;
	dst_nl_addr.nl_groups = 0;
	
	netlink_fd = socket(PF_NETLINK, SOCK_RAW, NETLINK_SERIAL_DAEMON);
	bind(netlink_fd, (struct sockaddr *)&src_nl_addr, sizeof(struct sockaddr_nl));
	
	serial_daemon_sendmsg(netlink_fd, &dst_nl_addr, CMD_ATTACH);
#endif /* SERIAL_DAEMON */
...
#ifdef SERIAL_DAEMON
		FD_SET(netlink_fd,&rdfdset);
		if (netlink_fd > G.maxfd)
				G.maxfd = master_fd;
#endif /* SERIAL_DAEMON */
...
#ifdef SERIAL_DAEMON  /*add by chen yingtao 2014-03-07*/
		if (FD_ISSET(netlink_fd, &rdfdset)) 
		{
			/* printf("netlink get data"); */
			serial_daemon_recvmsg(netlink_fd);
		}
#endif /* SERIAL_DAEMON */
